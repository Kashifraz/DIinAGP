 ...
 * Textarea field class
 */
class Form_Plugin_Textarea_Field extends Form_Plugin_Base_Field {
    ...
    public function render($args = array()) {
        ...
        $html .= sprintf('<textarea %s>%s</textarea>', $attributes, esc_textarea($value));
    ...
    public function render_config_form() {
        ...
        // Rows
        $html .= '<div class="form-group mb-3">';
        $html .= '<label for="field-rows" class="form-label">' . __('Number of Rows', 'form-plugin') . '</label>';
        $html .= sprintf(
            '<input type="number" id="field-rows" name="rows" class="form-control" value="%s" min="1" max="20">',
            esc_attr($this->get_value('rows'))
        );
        $html .= '</div>';
        ...
        // Min length
        $html .= '<div class="form-group mb-3">';
        $html .= '<label for="field-min-length" class="form-label">' . __('Minimum Length', 'form-plugin') . '</label>';
        $html .= sprintf(
            '<input type="number" id="field-min-length" name="min_length" class="form-control" value="%s" min="0" placeholder="%s">',
            esc_attr($this->get_value('min_length')),
    ...
    public function get_label() {
        return __('Textarea Field', 'form-plugin');
    }
